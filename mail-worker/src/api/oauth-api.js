...
// 处理LinuxDo OAuth回调（GET请求）
app.get('/oauth/linuxdo/callback', async (c) => {
	// 从URL参数中获取code
	const code = c.req.query('code');
	// 重定向到前端登录页面，带上code参数
	const origin = new URL(c.req.url).origin;
	return c.redirect(`${origin}/linuxdo/callback?code=${code}`, 302);
});
...
// 处理LinuxDo解绑
app.delete('/oauth/unbindLinuxDo', async (c) => {
	const userContext = c.get('userContext');
	await oauthService.unbindLinuxDo(c, userContext.userId);
	return c.json(result.ok())
})
