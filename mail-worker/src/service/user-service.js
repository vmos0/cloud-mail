...
const userService = {
	...
	async loginUserInfo(c, userId) {
		...
		const [account, roleRow, permKeys, githubOauth, linuxdoOauth] = await Promise.all([
		...
		// 添加绑定信息
		if (githubOauth) {
			user.githubUsername = githubOauth.username;
			user.githubAvatar = githubOauth.avatar;
		} else {
		...
		if (linuxdoOauth) {
			user.linuxdoUsername = linuxdoOauth.username;
			user.linuxdoAvatar = linuxdoOauth.avatar;
		} else {
			user.linuxdoUsername = null;
			user.linuxdoAvatar = null;
		}
