...
const oauthService = {
	...
	async linuxDoLogin(c, params) {
		...
		// 如果没有找到用户，尝试查找是否有其他用户绑定了该LinuxDo账户
		if (!userRow || oauthRow.userId === 0) {
			// 检查是否有用户已经绑定了该LinuxDo账户
			const existingBinding = await userService.selectByOauthUserId(c, userInfo.oauthUserId, 0);
			if (existingBinding) {
	...
	async githubLogin(c, params) {
		...
		if (!userRow || oauthRow.userId === 0) {
			...
			const existingBinding = await userService.selectByOauthUserId(c, userInfo.oauthUserId, 1);
			if (existingBinding) {
				userRow = existingBinding;
				// 更新OAuth记录的userId
				await orm(c).update(oauth).set({ userId: userRow.userId }).where(eq(oauth.oauthId, oauthRow.oauthId)).run();
	...
	async unbindLinuxDo(c, userId) {
		await orm(c).delete(oauth).where(and(eq(oauth.userId, userId), eq(oauth.platform, 0))).run();
	},

	async unbindGithub(c, userId) {
		await orm(c).delete(oauth).where(and(eq(oauth.userId, userId), eq(oauth.platform, 1))).run();
	},
}
